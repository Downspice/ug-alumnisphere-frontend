    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#c2c2c2] mt-1">
          Open reports from posts and comments. Marking reviewed does not delete the original content.
        </p>
      </div>
      {loading ? (
        <LoadingState variant="rows" count={3} message="Loading reports..." />
      ) : error ? (
        <ErrorState title="Could not load reports" message={error} onRetry={() => refetch()} />
      ) : reports.length === 0 ? (
        <EmptyState title="No open reports" description="When someone reports a post or comment, it appears here." />
      ) : (
          ...
          {reports.map((report) => (
            <article key={report.id} className="frosted-glass-card p-4 space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-medium capitalize">{report.targetType}</div>
              ...
              <Button type="button" size="sm" disabled={reviewing} onClick={() => reviewReport(report.id)}>
                Mark reviewed
              </Button>
            </article>
          ))}
