...
import { AnalyticsBarChart, AnalyticsLineChart } from "@/components/domain/analytics-charts";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#c2c2c2] mt-1">
          Charts are built with Recharts from live Mongo aggregations. Empty series mean that collection has no documents yet.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <ErrorState title="Could not load analytics" message={error} onRetry={() => refetch()} />
      ) : !analytics ? (
        <EmptyState title="No analytics yet" description="Create users, jobs, events, or contributions, then return." />
      ) : (
        <div className="space-y-4">
          ...
          <section className="frosted-glass-card p-5 space-y-3">
            ...
            {analytics.contributionsByMonth.length === 0 ? (
              ...
              <AnalyticsLineChart data={analytics.contributionsByMonth} label="GHS recorded" />
            )}
          </section>
        </div>
      )}
