...
export const mentorshipRequestSchema = z.object({
  message: z.string().trim().min(20, "Explain what you need in at least 20 characters").max(1000),
});
...
export const eventSchema = z.object({
  title: z.string().trim().min(3, "Title is required").max(140),
  description: z.string().trim().min(20, "Describe the event in at least 20 characters").max(4000),
