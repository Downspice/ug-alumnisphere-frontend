...
export function useCommunities(search?: string, mine?: boolean) {
  const { data, loading, error, refetch } = useQuery<{ communities: Community[] }>(GET_COMMUNITIES, {
...
export function useCommunity(id?: string) {
  const { data, loading, error, refetch } = useQuery<{ community: Community | null }>(GET_COMMUNITY, {
    variables: { id },
    skip: !id,
    errorPolicy: "all",
  });
...
export function useCommunityMembers(communityId?: string) {
  const { data, loading, error, refetch } = useQuery<{ communityMembers: CommunityMember[] }>(
    GET_COMMUNITY_MEMBERS,
    {
...
export function useCommunityActions(communityId?: string) {
  ...
  const [create, createState] = useMutation<{ createCommunity: Community }>(CREATE_COMMUNITY, {
    refetchQueries: [{ query: GET_COMMUNITIES }, { query: GET_COMMUNITIES, variables: { mine: true } }],
    onCompleted: () => toast.success("Community created"),
    onError: (err) => toast.error("Could not create community", { description: err.message }),
  });
  const [join, joinState] = useMutation(JOIN_COMMUNITY, {
    ...
    onCompleted: (data) => {
      ...
      toast.success(
        community?.isPrivate && !community.myRole ? "Join request sent" : "You joined the community"
      );
    },
  ...
  const [review, reviewState] = useMutation(REVIEW_JOIN_REQUEST, {
    ...
    onError: (err) => toast.error("Could not review request", { description: err.message }),
  });
  ...
  return {
    createCommunity: (input: { name: string; description?: string; isPrivate?: boolean }) =>
      create({ variables: { input } }),
    joinCommunity: (id: string) => join({ variables: { id } }),
    leaveCommunity: (id: string) => leave({ variables: { id } }),
    reviewJoinRequest: (id: string, approve: boolean) => review({ variables: { id, approve } }),
